...
class Unit:
    def __init__(self, ai, id, nome,race):
        ...
        self.basestats=[self.maxhp, self.atk, self.totatkpts, self.totmovepts,self.atkrange]
        self.inventory={'head': '',
                        'body': '',
                        'lhand': '',
                        'rhand': '',}
    ...
    def draw(self, screen):
        ...
        if self.rectMask.collidepoint(mouse_pos[0], mouse_pos[1]) == True:
            for i in self.glow:
                if i != None:
                    for dot in i:
                        pygame.draw.rect(screen, (247, 247, 247), pygame.Rect(dot[0]+self.center[0]-Main.resizable_layer_x, dot[1]+self.center[1] -Main.resizable_layer_y, 1, 1))
                        pygame.draw.rect(screen, (247, 247, 247), pygame.Rect(dot[0]+self.center[0]+1-Main.resizable_layer_x, dot[1]+self.center[1]-Main.resizable_layer_y, 1, 1))
                        pygame.draw.rect(screen, (247, 247, 247), pygame.Rect(dot[0]+self.center[0]-1-Main.resizable_layer_x, dot[1]+self.center[1]-Main.resizable_layer_y, 1, 1))
                        pygame.draw.rect(screen, (247, 247, 247), pygame.Rect(dot[0]+self.center[0]-Main.resizable_layer_x, dot[1]+self.center[1]-1-Main.resizable_layer_y, 1, 1))
                        pygame.draw.rect(screen, (247, 247, 247), pygame.Rect(dot[0]+self.center[0]-Main.resizable_layer_x, dot[1]+self.center[1]+1-Main.resizable_layer_y, 1, 1))
        #disegno l'omino
        ...
        for i in self.img:
            if i !=None:
                ...
                screen.blit(i, (self.center[0]-Main.resizable_layer_x, self.center[1]-Main.resizable_layer_y))
    ...
    def drawHpBar(self, screen,max,height,x,y):
        ...
        if self.ai==True:
            pygame.draw.rect(
                screen, (242, 39, 39), (x-Main.resizable_layer_x, y+4-Main.resizable_layer_y, rectWidth, height),
                0,
                3
            )
            pygame.draw.rect(
                screen, (34, 32, 52  ), (x-Main.resizable_layer_x, y+4-Main.resizable_layer_y, rectMaxLength, height),
                2,
                3
            )
        else:
            pygame.draw.rect(
                screen, (3, 166, 74), (x-Main.resizable_layer_x, y+4-Main.resizable_layer_y, rectWidth, height),
                0,
                3
            )
            pygame.draw.rect(
        ...
        text=Main.font1.render(str(self.hp)+"/"+str(self.maxhp), True, (29,12,28))
        text=pygame.transform.scale(text, (rectMaxLength/2,10))
        screen.blit(text, (x+rectMaxLength/4-Main.resizable_layer_x, y-6-Main.resizable_layer_y))
    ...
    def createMask(self):
        self.rectMask = pygame.Rect(
            self.center[0], self.center[1], self.img[1].get_width(), self.img[1].get_height()
        )
    ...
    def select(self):
        ...
        if self.ai == False and self.moving==False and Main.controller.actingUnit == None:
            # SELEZIONE
            mouse_pos = pygame.mouse.get_pos()
                                    ...
                                    if (
                                        ...
                                        if unit.activated == False:
                                            if Main.controller.selectedd == self:
                                                self.deselect()
        # SELEZIONE PER AI
        elif self.ai == True and self.moving==False and Main.controller.actingUnit == None:
    ...
    def moveAnimation(self):
        ...
        if self.move_target!=None:
            ...
            if Main.controller.actingUnit==None or Main.controller.actingUnit==self:
                ...
                if length!=0 and length > pygame.math.Vector2(direction_x/ length, direction_y/ length).length() * Main.speed :
                    Main.controller.actingUnit=self
                    path = pygame.math.Vector2(direction_x/ length, direction_y/ length)
                    self.moving=True
    ...
    def calculateMidPoint(self):
        ...
        middle=[(self.x + self.attacked_target.x) / 2, (self.y + self.attacked_target.y) / 2]
        return middle
        ...
        if self.attacked_target!=None: 
            ...
            #print(self.x, self.middle[0],self.attacked_target.x)
                ...
                if self.attacked_target_returning==False:
                    ...
                    direction=pygame.math.Vector2(self.middle[0] - self.x, self.middle[1] - self.y)
                    ...
                    direction=pygame.math.Vector2(self.start_x-self.x,self.start_y-self.y)
                    length=pygame.math.Vector2(direction).length()
                    #print (length)
                    ...
                    if length>pygame.math.Vector2(direction[0],direction[1]).length()+animSpeed:
                            
                            #print('3')
                            self.x+=direction[0]*animSpeed
                            self.y+=direction[1]*animSpeed

                    elif length<=pygame.math.Vector2(direction[0],direction[1]).length()+animSpeed:
    ...
    def aiEndTurn(self):
        ...
        enemy=self.aiCalculateNearestEnemy()
        if enemy.parentcell.dts > self.atkrange and self.movepts == 0 and Main.controller.actingUnit == None and Main.controller.ai_selected == self:
    ...
    def applyRaceModifiers(self):
        ...
        if self.race=='human':
            ...
            self.img[1]=pygame.image.load('./media/races/goblin/goblin_base_body_0.png')
            self.orig_img[1]=self.img[1]   
            self.img[2]=pygame.image.load('./media/races/goblin/goblin_base_head_0.png')
            self.orig_img[2]=self.img[2]
            self.atkpts += 1
            self.movepts += 2
            self.totmovepts += 2
